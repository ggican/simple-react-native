...
const Login = () => {
  ...
  const onGetDataLogin = () => {
    if (isLoading) {
      setLoading(true);
      service.heroTopClear({dispatch});
      service.heroTop({dispatch});
      setFirstGet(false);
    }
  };

  const onGetDataLoginCallback = useCallback(onGetDataLogin, []);

  React.useEffect(() => {
    if (isLoading) {
      onGetDataLoginCallback();
    }
    return () => {};
  }, []);
